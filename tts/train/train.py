...
def process_batch(model, optimizer, criterion, batch, train=True):
    ...
    batch_size, frames_lengths, _ = melspecs.shape
    mask = torch.arange(num_frames).view(1, num_frames) >= \
           melspec_lengths.view(batch_size, 1)

    stop_labels = torch.arange(num_frames).view(1, num_frames) >= \
...
def process_epoch(model, optimizer, criterion, spectrogramer, loader, params, train=True):
    ...
    for raw_batch in loader:
        ...
        melspec_lengths = get_spectrogram_lengths(audio_lengths)
